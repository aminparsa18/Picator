...
public partial class ProfileSettingsViewModel : ViewModelBase
{
    ...
    private async Task SetPhotoAsync()
    {
        if (!Validate())
            return;
        await PopupNavigation.Instance.PushAsync(new WaitingView("Uploading Image..."));
        try
        {
    ...
    private async Task AvatarSelectedAsync()
    {
        if (!Validate())
            return;
        await PopupNavigation.Instance.PushAsync(new WaitingView("Changing profile picture"));
