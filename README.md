...
- `GameCreateService.Create()` is a stub (mapper calls commented out); only `CreateTimeGame` (the quick-match path) is functional.
...
- Round rotation (drawer sequencing), scoring calculation, timer enforcement, and guess detection in `ChatHub` are not implemented yet — the domain model supports them, but the service/hub logic is future work.
