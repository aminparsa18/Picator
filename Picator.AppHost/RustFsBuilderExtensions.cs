...
public static class RustFsBuilderExtensions
{
        ...
        int? consolePort = null)
    {
        ...
        // In local mode RustFS binds to its compiled default (9000) regardless of RUSTFS_ADDRESS.
        // In publish/K8s mode we need the container port to match the service port (9100) so the
        // Aspire-generated connection string resolves to the correct in-cluster address.
        int primaryTargetPort = builder.ExecutionContext.IsPublishMode
            ? RustFsResource.PrimaryTargetPort
            : 9000;
        ...
        var b = builder.AddResource(resource)
            ...
            .WithEnvironment("RUSTFS_ADDRESS", $":{primaryTargetPort.ToString()}")
    ...
    : ContainerResource(name), IResourceWithConnectionString
{
    public ReferenceExpression ConnectionStringExpression =>
        ReferenceExpression.Create(
            $"Endpoint={this.GetEndpoint(PrimaryEndpointName)};AccessKey={AccessKey};SecretKey={SecretKey}");
    ...
    internal const int PrimaryTargetPort = 9100;
