    ...
    internal static string GetBasePath(string applicationId)
    {
        ...
        if (string.IsNullOrWhiteSpace(BasePath))
            // Gets full path based on device type.
				...
				BasePath = Foundation.NSSearchPath.GetDirectories(Foundation.NSSearchPathDirectory.CachesDirectory, Foundation.NSSearchPathDomain.User)[0];
#elif __ANDROID__
            BasePath = Android.App.Application.Context.CacheDir.AbsolutePath;
#else
